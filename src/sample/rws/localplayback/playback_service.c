...
static P_FUN_EXT_SERVER_GENERAL_CB  p_playback_extended_cb = NULL;
#else
                                                   ...
                                                   uint8_t *p_value, P_FUN_WRITE_IND_POST_PROC *p_write_ind_post_proc)
{
    ...
    {
        cause = app_playback_trans_ble_handle_cp_req(conn_id, length, p_value);
    }
    else
    {
...
T_SERVER_ID playback_add_service(void *p_func)
{
    ...
    p_playback_extended_cb = (P_FUN_EXT_SERVER_GENERAL_CB)p_func;
...
void playback_service_send_notification(uint8_t conn_id, uint8_t *p_data, uint16_t data_len)
{
    ...
    server_ext_send_data(le_get_conn_handle(conn_id), L2C_FIXED_CID_ATT, srv_id_local,
                         BLE_SERVICE_CHAR_PLAYBACK_NOTIFY_INDEX, p_data, data_len,
